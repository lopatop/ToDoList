...
import {createTaskTC, deleteTasksTC, TasksPropsType, tasksReducer, updateTaskTC} from "../tasks-slice"
...
test("correct task should be deleted", () => {
  ...
  const endState = tasksReducer(
      startState,
      deleteTasksTC.fulfilled({todolistId: "todolistId2", taskId: "2"}, "requestId", {
...
test("correct task should be created at correct array", () => {
  ...
  const endState = tasksReducer(
    startState,
    createTaskTC.fulfilled({task:task, todolistId:"todolistId2"}, "requestId", { todolistId: "todolistId2", title: "juice" }),
  )
...
test("array should be created for new todolist", () => {
  //создание места для тасок при создании тудулиста
...
test("property with todolistId should be deleted", () => {
  //удаление тудулиста вместе с тасками
  const endState = tasksReducer(
    startState,
